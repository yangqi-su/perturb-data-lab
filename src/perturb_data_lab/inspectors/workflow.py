...
def run_batch(
    ...
    """Thin batch wrapper: inspect each configured dataset.
    ...
    index; the authoritative artifact for any dataset is its inspection summary
    (dataset-summary.yaml) on disk, not the manifest itself.

    Each manifest record points to exactly one inspection summary path.
    """
    ...
    manifest = InspectionBatchManifest(
        ...
        records=tuple(
            InspectionBatchRecord(
                ...
                inspection_summary=str(artifact.inspection_summary),
