...
def write_zarr_federated(
    ...
    *,
    ...
    if _is_last_chunk:
        ...
        _writer_state["row_offsets_zarr"]["row_offsets"].resize(
            (_writer_state["row_count"] + 1,)
        )
...
def write_zarr_aggregate(
    ...
    **kwargs: Any,
    ...
    """Stream ChunkBundle to Zarr aggregate format with stateful open-zarr groups.
    ...
    via slice assignment.
    On last call (_is_last_chunk=True): resize arrays to exact final sizes,
    write aggregated-meta.json, and return None for writer_state.
    ...
    _is_last_chunk : bool
        True when this is the final chunk — triggers final resize,
        aggregated-meta.json write, and returns None for writer_state.
    ...
    if ro_end > _writer_state["row_offsets_zarr"]["row_offsets"].shape[0]:
        _writer_state["row_offsets_zarr"]["row_offsets"].resize((ro_end + 1000,))
    _writer_state["row_offsets_zarr"]["row_offsets"][ro_start:ro_end] = (
        bundle.indptr + current_nnz
    )
    ...
    # Write chunk data via slice assignment.
    _writer_state["indices_zarr"]["indices"][current_nnz : current_nnz + chunk_nnz] = (
        bundle.indices
    )
    _writer_state["counts_zarr"]["counts"][current_nnz : current_nnz + chunk_nnz] = (
        bundle.counts
    )
    ...
    if _is_last_chunk:
        # Resize to exact final sizes.
        _writer_state["indices_zarr"]["indices"].resize(
            (_writer_state["global_nnz"],)
        )
        _writer_state["counts_zarr"]["counts"].resize(
            (_writer_state["global_nnz"],)
        )
        _writer_state["row_offsets_zarr"]["row_offsets"].resize(
            (_writer_state["row_count"] + 1,)
        ...
        # Write aggregated-meta.json with final dimensions.
        with open(meta_path, "w") as f:
            json.dump(
                {
                    "total_rows": _writer_state["row_count"],
                    "total_nnz": _writer_state["global_nnz"],
                    "indices_path": str(indices_path),
                    "counts_path": str(counts_path),
                    "row_offsets_path": str(row_offsets_path),
                },
